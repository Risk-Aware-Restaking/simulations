...
def generate_network_graph(
    ...
    for deg in sorted(
        possible_degrees, reverse=True
    ):  # Start with highest degrees to prioritize fulfilling condition
        required_for_this_deg = int(2 * deg)

        # If we have enough services left and this degree's demand can fit
        if (
            service_num >= required_for_this_deg
            and len(service_degrees) + required_for_this_deg <= service_num
        ):
            service_degrees.extend([deg] * required_for_this_deg)
        elif (
            len(service_degrees) < service_num
        ):  # If we have some slots left, but not enough for full demand
...
if __name__ == "__main__":
    ...
    print(
        f"  Generated Rewards (for services): {[round(r, 2) for r in generated_rewards]}"
